...
string ReceiveAns (CClientSocket &sock){
  ...
  try{
	...
	Log::instance().Trace( 0, "ReceiveAns catched exception %s ", e.what() );
  }catch( ... ){
	Log::instance().Trace( 0, "ReceiveAns catched unknown exception");
  }
...
void ReceiveAnsLIST (CClientSocket &sock, IN const char* strAddress, IN StoreDataFunc pStoreFunc, string strPath,
  ...
  try{ 
	...
	Log::instance().Trace( 0, "FTP::ReceiveAnsLIST: Catched exception for address  %s: %s", strAddress, e.what() );
  }catch( ... ){
	Log::instance().Trace( 0, "FTP::ReceiveAnsLIST: Catched unknown exception for address %s", strAddress );
  }	
}
...
unsigned int SendCommand (std::string Command, CClientSocket &sock)
{
  ...
  catch( std::exception& e ){ Log::instance().Trace( 0, "FTP::SendCommand: catched exception %s",e.what() ); }
  catch( ... ){ Log::instance().Trace( 0, "FTP::SendCommand: Catched unknow exception" ); }
...
DllExport bool Scan( IN const char* strAddress, IN StoreDataFunc pStoreFunc, IN HANDLE hCancelEvent )
{
  try{
	...
	Log::instance().Trace( 0, "FTP::Scan: %s:21 ", strAddress );
	SendCommand (strCommand[7], ctrl_sock);
	return true;
  }catch( std::exception& e )
	{
	  Log::instance().Trace( 0, "FTP::Scan: Catched exception for address %s: %s", strAddress, e.what() );
	}catch( ... )
	{
	  Log::instance().Trace( 0, "FTP::Scan: Catched unknown exception for address %s", strAddress );
	}
  return false;
}
