...
void CScheduler::OnStartScan()
{
	try{
		...
		for( std::map< std::string, SmartPtr< CAgentHandler > >::iterator ItDiv = m_mapAgentsContainer.begin(); ItDiv != m_mapAgentsContainer.end(); ItDiv++, i++ )
		{
			iStartPos = iEndPos;
			...
			mapRanges[ ItDiv->first ] = vecRange;
			ItDiv->second->BeginScan( vecRange );
			vecRange.clear();
		}
